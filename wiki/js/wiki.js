...
async function init() {
  try {
    ...
    // ローカルファイルとして開いた場合はfetchが失敗するのでフォールバック
    document.getElementById('article-content').innerHTML =
      '<p style="font-family:sans-serif;color:#c00">⚠ ローカルで直接開く場合は、ローカルサーバー経由でアクセスしてください。</p>' +
      '<p style="font-family:sans-serif;font-size:13px;margin-top:8px">例: <code>python3 -m http.server 8000</code> を実行して <a href="http://localhost:8000">localhost:8000</a> にアクセスしてください。</p>';
    return;
  }
